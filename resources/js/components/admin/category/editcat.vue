...
<div class="text-center  py-4 card">
 ...
 <h3  class="my-4  h3  font-weight-bold">Add Categories  </h3>	
	...
	<form @submit.prevent="upCategory" @keydown="form.onKeydown($event)">
    ...
    <label  for="inactive" > Inactive </label>
      ...
      <input type="radio" class="mx-3 form-check-input" id="active"
      value="1" v-model="form.status" />
...
export default {
	...
	axios.get('http://localhost/laravel_projects/Vue_eCommerce/public/admin/edit-category/'+id).then( function(response) {
	t.form.catName = response.data.data[0].cat_name;
	t.form.status = response.data.data[0].status;
	...
	async upCategory(){ 
	var id=this.$route.params.id; var t=this;
	const response = await this.form.post('http://localhost/laravel_projects/Vue_eCommerce/public/admin/upcat/'+id)
